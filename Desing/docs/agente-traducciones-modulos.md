...
## 6. Generador `_gen_<módulo>_resources.js`

1. Duplicar un script existente → `_gen_<modulo>_resources.js`; ajustar nombre de clase/archivos `.resx`.
2. Mantener **paridad `es` / `en`** en cada clave nueva.
3. Desde `Desing/`: `node _gen_<modulo>_resources.js`.
...
- [ ] Crear **`_gen_<modulo>_resources.js`**, ejecutar `node`, verificar entradas en **`Design.csproj`** (`EmbeddedResource` + `Compile` del Designer).
...
| Ejemplo plantilla empresa | `Views/TSql_Company/*`, `Controllers/TSql_CompanyController.cs`, `_gen_company_resources.js` |
