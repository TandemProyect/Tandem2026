...
function InitDesaint3d(_estimationId, _estimationName, _sizeX, _sizeY, _camera, _linkEnvironment, _LinkIdEnvironmentOrbitValue) {
    ...
    createEscena();
    createRender();
    if (cameraPerspectiveInitialPosition === null) {
        setInitialPerspectiveValues();
    }
...
function changeCamera(camType) {
    cameraTypeId = camType;
    if (cameraTypeId === 1) {
        orthographicCamera.controls.saveState();
        perspectiveCamera.controls.reset();

    } else {
        perspectiveCamera.controls.saveState();
        orthographicCamera.controls.reset();
...
function animate() {
    ...
    if (cameraTypeId === 1) {
        ...
        /*       $("#btnPerspectiveCamera").addClass("buttonTurnedOn");*/
    }
    else {
        orthographicCamera.camera.left = getContainerWidth() / -camSettings.orthographicScale;
        orthographicCamera.camera.right = getContainerWidth() / camSettings.orthographicScale;
        orthographicCamera.camera.top = getContainerHeight() / camSettings.orthographicScale;
        orthographicCamera.camera.bottom = getContainerHeight() / -camSettings.orthographicScale;
        orthographicCamera.camera.updateProjectionMatrix();
        camera = orthographicCamera.camera;
        renderer.render(scene, camera);
        /*  $("#btnOrthographicCamera").addClass("buttonTurnedOn");*/
    }
    ...
    function setView(view) {
        ...
        if (cameraTypeId === 1) {
            ...
            switch (view) {
                case 'left':
                    orthographicCamera.camera.position.x = cameraLeftPosition.x - camSettings.Factor * 4;
                    orthographicCamera.camera.position.y = cameraLeftPosition.y;
                    orthographicCamera.camera.position.z = cameraLeftPosition.z;
                    break;
                case 'top':
                    orthographicCamera.camera.position.x = cameraTopPosition.x;
                    orthographicCamera.camera.position.y = cameraTopPosition.y + camSettings.Factor * 4;
                    orthographicCamera.camera.position.z = cameraTopPosition.z;
                    break;
                case 'front':
                    orthographicCamera.camera.position.x = cameraFrontPosition.x;
                    orthographicCamera.camera.position.y = cameraFrontPosition.y;
                    orthographicCamera.camera.position.z = cameraFrontPosition.z + camSettings.Factor * 4;
                    break;
                case '3d':
                    orthographicCamera.camera.position.x = cameraOrthographic3dPosition.x - camSettings.Factor;
                    orthographicCamera.camera.position.y = cameraOrthographic3dPosition.y + camSettings.Factor;
                    orthographicCamera.camera.position.z = cameraOrthographic3dPosition.z + camSettings.Factor;
                    break;
            }
            orthographicCamera.camera.zoom = 0.2;
            orthographicCamera.camera.updateProjectionMatrix();
            orthographicCamera.controls.target.x = cameraDefaultTarget.x;
            orthographicCamera.controls.target.y = cameraDefaultTarget.y;
            orthographicCamera.controls.target.z = cameraDefaultTarget.z;
            orthographicCamera.controls.update();
        }
    }
...
function createMaterial() {
    materialDimWall = new THREE.MeshLambertMaterial({ color: 0x2300ff });
    materialDim = new THREE.MeshLambertMaterial({ color: 0x2300ff });
    if (LinkEnvironment === 9) {
        materialDimWall = new THREE.MeshLambertMaterial({ color: 0xffe000 });
        materialDim = new THREE.MeshLambertMaterial({ color: 0xffe000 });
    }
    ...
    if (LinkEnvironment === 2) {
        materialWall = new THREE.MeshLambertMaterial({ color: 0xf1f1f5 });
    }
    if (LinkEnvironment === 9) {
        materialWall = new THREE.MeshLambertMaterial({ color: 0xf1f1f5 });
    }
...
function createCamara() {
    ...
    function createControlsForCamera(camObject, camType) {
        var controlOrbit = new THREE.OrbitControls(camObject);
        controlOrbit.target = new THREE.Vector3(
            cameraDefaultTarget.x,
            cameraDefaultTarget.y,
            cameraDefaultTarget.z);
        controlOrbit.enabled = true;
        controlOrbit.enableZoom = true;
        controlOrbit.enablePan = true;
        controlOrbit.enableRotate = true;
        controlOrbit.enableDamping = false;
        controlOrbit.autoRotate = false;
        controlOrbit.screenSpacePanning = true;
        controlOrbit.keyPanSpeed = currentPanSpeed;
        controlOrbit.rotateSpeed = currentZoomSpeed;
        controlOrbit.zoomSpeed = currentZoomSpeed;
        controlOrbit.panSpeed = currentZoomSpeed;
        return controlOrbit;
    }
    camera.position.set(cameraDefaultPosition.x, cameraDefaultPosition.y, cameraDefaultPosition.z);
...
function createortogonalCamera() {
    var cameraOrto = new THREE.OrthographicCamera(
        getContainerWidth() / -camSettings.orthographicScale,
        getContainerWidth() / camSettings.orthographicScale,
        getContainerHeight() / camSettings.orthographicScale,
        getContainerHeight() / -camSettings.orthographicScale,
        camSettings.near,
        camSettings.far);
    camera = createControlsForCameraOrb(camera, 2);
    camera = new OrthographicCamera();
    camera.camera = cameraOrto;
    camera.camera.position.x = cameraOrthographicInitialPosition.x;
    camera.camera.position.y = cameraOrthographicInitialPosition.y;
    camera.camera.position.z = cameraOrthographicInitialPosition.z;
    camera.camera.zoom = oZoom;
    camera.camera.updateProjectionMatrix();
    camera.controls = controlsorthographic;
    camera.controls.update();
    camera.controls.saveState();
}
function createControlsForCameraOrb(camObject, camType) {
    control = new THREE.OrbitControls(camObject);
    if (camType === 1) {
        control.target = new THREE.Vector3(
            cameraPerspectiveInitialTarget.x,
            cameraPerspectiveInitialTarget.y,
            cameraPerspectiveInitialTarget.z);
    } else {
        control.target = new THREE.Vector3(
            cameraOrthographicInitialTarget.x,
            cameraOrthographicInitialTarget.y,
            cameraOrthographicInitialTarget.z);
    }
...
function createAxes() {
    // axesHelper
    var arrowPos = new THREE.Vector3(0, 5, 0);
    scene.add(new THREE.ArrowHelper(new THREE.Vector3(5, 0, 0), arrowPos, 160, 0xffe000, 20, 10));
    scene.add(new THREE.ArrowHelper(new THREE.Vector3(0, 5, 0), arrowPos, 160, 0xffe000, 20, 10));
    scene.add(new THREE.ArrowHelper(new THREE.Vector3(0, 0, 5), arrowPos, 160, 0xffe000, 20, 10));
}
function createEnvironment(linkEnvironment, LinkIdEnvironmentOrbitValue) {
    ...
    //    textureSky = loader.load("../../Content/images/desing/sky.png");
    //var materialSky = new THREE.MeshPhongMaterial({ map: textureSky, });

    //var sky2 = new THREE.Mesh(skyGeo, materialSky);
...
function getContainerWidth() {
    return $('containerDesign').width();
}

function getContainerHeight() {
    return $('containerDesign').height();
}
