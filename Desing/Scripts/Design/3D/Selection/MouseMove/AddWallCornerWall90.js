function AddWallCornerWall90() {
    if (ActiveAddCorner === "AddCorner90") {
        ...
        if (intersects.length > 0) {
            ...
            meshEsq90.visible = true;
            meshEsq90.position.copy(intersect.point).add(intersect.face.normal);
            meshEsq90.position.divideScalar(10).floor().multiplyScalar(10).addScalar(10);
            if (isYColision === true) {
                meshEsq90.position.x = xColision;
                meshEsq90.position.z = yColision;
                isYColision = false;
                xColision = 0;
                yColision = 0;
                ActionDbl = "AddCorner90";
                ActiveAddCorner = "";
                
                return;
            }
            else {
                yColision = meshEsq90.position.y;
                xColision = meshEsq90.position.x;
            }
        }
