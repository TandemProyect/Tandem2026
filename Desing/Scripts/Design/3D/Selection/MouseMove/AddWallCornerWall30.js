function AddWallCornerWall30() {
    if (isYColision === false && ActiveAddCorner === "AddCorner30_exit") {
        ActiveAddCorner = "AddCorner30";
    }
    if (ActiveAddCorner === "AddCorner30")
    {
        ...
        if (intersects.length > 0) {
            ...
            meshEsq30.position.copy(intersect.point).add(intersect.face.normal);
            meshEsq30.position.divideScalar(10).floor().multiplyScalar(10).addScalar(10);
            if (isYColision === true) {
                meshEsq30.position.x = xColision;
                meshEsq30.position.z = yColision;
                xColision = 0;
                yColision = 0;
                ActiveAddCorner = "AddCorner30_exit";
                if (TypeConetion === "Wall_R000") {
                    ActionDbl = "AddCorner30_00";
                    return;
                }
                if (TypeConetion === "Wall_R900") {

                    ActionDbl = "AddCorner30_90";
                    return;
                }
            }
            else {
                yColision = meshEsq30.position.y;
                xColision = meshEsq30.position.x
            }
        }
