...
{
    public class Corner70_00_Esq : BaseController
    {
        ...
        internal static List<ModelRenderElement> setdListElement(long type, DAL.TSql_DefaultDesign currentDefaultDisign, long DataHeight, long dataWith, long datalong, long dataCordenadX, long dataCordenadY, long? DataWithOtherCorner)
        {
            ...
            SedPanels(type, currentDefaultDisign, DataHeight, datalong, dataCordenadX, dataCordenadY, dataWith, DataWithOtherCorner, ListRenderElement);
        ...
        private static void SedPanels(long type, DAL.TSql_DefaultDesign currentDefaultDisign, long DataHeight, long datalong, long dataCordenadX, long dataCordenadY, long dataWith, long? DataWithOtherCorner, List<ModelRenderElement> ListRenderElement)
        {
            ...
            long _DataWithOtherCorner = 0;
            if (DataWithOtherCorner != null)
            {
                _DataWithOtherCorner = (long)DataWithOtherCorner;
            }
            dataCordenadX = (dataCordenadX + (datalong / 10)) - _DataWithOtherCorner / 10;
            ...
            if (n >= 1)
            {
                ...
                for (int i = 0; i < n; i++)
                {
                    if (IsRemateX < 10)
                    {
                        var distEsq = 30;
                        var typeNoRemate = 90;
                        CommonElement.SedUnionVertical(typeNoRemate, PanelPerfil, 0, dataCordenadX - distEsq, 0, dataCordenadY + distEsq - PanelPerfil, nHeight + 45, ListRenderElement, "");
                        CommonElement.SedUnionVertical(typeNoRemate, PanelPerfil, 0, dataCordenadX - distEsq, 0, dataCordenadY + distEsq - PanelPerfil, +135, ListRenderElement, "");
                        CommonElement.SedUnionVertical(typeNoRemate, PanelPerfil, 0, dataCordenadX - distEsq, 0, dataCordenadY + distEsq - PanelPerfil, +225, ListRenderElement, "");
                    }
                    if (i == 0) { Elevation = 45; }
                    else
                    {
                    ...
                    element.LongDimTypeHorizontal = (long?)300;
            ...
            IsRemateX = datalong - _DataWithOtherCorner - 300;
            IsRemateY = GetLongRemateY(dataWith + 300);
            //if (IsRemateX > 10)
            //{
            //    var ListRenderElementRemate0 = Common.Remate0.setdListElement(IsRemateX, DataHeight, dataWith, datalong, dataCordenadXRemate, dataCordenadYRemate);
            //    if (ListRenderElementRemate0 != null)
            //    {
            //        foreach (var item in ListRenderElementRemate0)
            //        {
            //            ModelRenderElement element = new ModelRenderElement();
            ...
            //if (IsRemateY > 10)
            //{
            //    var ListRenderElementRemate90 = Common.Remate90.setdListElement(IsRemateX, IsRemateY, DataHeight, dataWith, datalong, dataCordenadXRemate, dataCordenadY);
            //    if (ListRenderElementRemate90 != null)
            //    {
            //        foreach (var item in ListRenderElementRemate90)
            //        {
            //            ModelRenderElement element = new ModelRenderElement();
            //            element.IdElement = item.IdElement;
            //            element.CodeName = item.CodeName;
        ...
        private static long GetLongRemateY(long v)
        {
            long vd = 300;
            if (v > 300) { vd = 450; }
            if (v > 450) { vd = 600; }
