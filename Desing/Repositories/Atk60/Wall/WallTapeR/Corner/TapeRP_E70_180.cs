    ...
    public class TapeRP_E70_180 : BaseController
    {
        ...
        internal static List<ModelRenderElement> setdListElement(long EndWallX, long nHeight, long DataHeight, long datalong, long dataWith, long dataCordenadY, long TypeH, bool IsCorner, long Position_Y, bool ExitingPanel2400)
        {
            ...
            if (WoodEternal != 0)
            {
                ...
                var ListRenderElementRemate0 = Common.Remate180.setdListElement(ListRenderElement, WoodEternal - 2, DataHeight, dataWith, datalong, EndWallX - 12, dataCordenadY);
            }
            if (nHeight != 0)
            {
            ...
            if (n >= 1)
            {
                for (int i = 0; i < n; i++)
                {
                    SedPanels270Corner(EndWallX, nHeight, RealDataHeight, datalong, dataCordenadY + WoodEternal, dataWith, ListRenderElement, IsCorner);
